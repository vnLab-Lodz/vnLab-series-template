...
exports.onPostBuild = (
  ...
  // WARNING: Modified `criticalFilePaths` in comparisson to the original plugin
  const pagePaths = precachePages.map(page =>
    page.split("public")[1].replace("index.html", "").replace("./html", "")
  )
  const pageResourcePaths = flat(
    precachePages.map(page => getResourcesFromHTML(page, pathPrefix))
  )
  const assetPaths = glob
    .sync(`${process.cwd()}/${rootDir}` + "/static/**/*.webp")
    .map(f => f.split("public")[1])
  const criticalFilePaths = _.uniq(
    pagePaths.concat(pageResourcePaths, assetPaths)
  )
  ...
  return workboxBuild
    ...
    .then(({ count, size, warnings }) => {
      ...
      const swAppend = fs
        ...
        // WARNING: Added `downloadPages` to the original plugin
        .replace(
          /const downloadPages = \[\]/g,
          `const downloadPages = ${JSON.stringify(criticalFilePaths)}`
        )
